const CardDB = {
            ...
            'c8': { id: 'c8', name: '水调歌头', cost: 2, type: '平', typeClass: 'type-ping', atkBase: 0, desc: `造成{V_ATK}点${K.SH}2次，抽取(1+轻功)张卡牌`, rarity: 'mid', cardType: '词牌', effect: () => {
                const en = State.combat.enemies[Combat._primaryTargetIdx()];
                Combat.dealDmgToEnemy(0, false, en);
                setTimeout(() => Combat.dealDmgToEnemy(0, false, en), 200);
                Combat.draw(1 + State.agi);
            } },
            ...
            'c15': { id: 'c15', name: '撩剑', cost: 1, type: '仄', typeClass: 'type-ze', atkBase: 4, isFixed: true, desc: `${K.FX}造成{V_ATK}点${K.SH}3次`, isAttack: true, rarity: 'low', cardType: '武卡', effect: () => {
                const en = State.combat.enemies[Combat._primaryTargetIdx()];
                Combat.dealDmgToEnemy(4, true, en);
                setTimeout(() => Combat.dealDmgToEnemy(4, true, en), 150);
                setTimeout(() => Combat.dealDmgToEnemy(4, true, en), 300);
            } },
            ...
            'c21': { id: 'c21', name: '双斩', cost: 1, type: '仄', typeClass: 'type-ze', atkBase: -3, desc: `造成{V_ATK}点${K.SH}两次`, isAttack: true, rarity: 'low', cardType: '武卡', effect: () => {
                const en = State.combat.enemies[Combat._primaryTargetIdx()];
                Combat.dealDmgToEnemy(-3, false, en);
                setTimeout(() => Combat.dealDmgToEnemy(-3, false, en), 200);
            } },
            ...
            'c23': { id: 'c23', name: '一转攻势', cost: 1, type: '仄', typeClass: 'type-ze', desc: `${K.GF}，本场战斗你每失去3点力，获得1点力`, rarity: 'mid', cardType: '功卡', effect: () => { State.combat.player.yiZhuan = true; Game.showToast('一转攻势：自此计数'); } },
            ...
            'c28': { id: 'c28', name: '白虹贯日', cost: 1, type: '平', typeClass: 'type-ping', toExhaust: true, atkBase: 5, desc: `打出后进入${K.CS}，造成{V_ATK}点${K.SH}，对血量最高敌人造成双倍${K.SH}`, isAttack: true, rarity: 'mid', cardType: '武卡', effect: () => {
                ...
                if (liv0.length === 1) {
                    const en = State.combat.enemies[liv0[0]];
                    Combat.dealDmgToEnemy(5, false, en);
                    setTimeout(() => Combat.dealDmgToEnemy(5, false, en), 120);
                    return;
                }
                Combat.dealDmgAll(5);
            ...
            'c29': { id: 'c29', name: '峨眉剑法', cost: 1, type: '仄', typeClass: 'type-ze', desc: `${K.GF}，本场战斗每打出3张武卡，${K.FX}抽取1张卡牌`, rarity: 'mid', cardType: '功卡', effect: () => { State.combat.player.emei = true; Game.showToast('峨眉剑法：剑路已明'); } },
            ...
            'c32': { id: 'c32', name: '波诡云谲', cost: 1, type: '平', typeClass: 'type-ping', toExhaust: true, desc: `打出后进入${K.CS}，将1张随机武卡加入手牌，该牌本回合耗气为0`, rarity: 'mid', cardType: '功卡', effect: () => {
                ...
                if (!pool.length) { Game.showToast('波诡云谲：竟无武卡可借'); return; }
                if (State.combat.hand.length >= 10) { Game.showToast('手牌已满，再也塞不下'); return; }
                const pick = pool[rand(0, pool.length - 1)];
                State.combat.hand.push({ cardId: pick, costOverride: 0 });
                Combat.renderHand();
                Game.showToast(`波诡云谲：${CardDB[pick].name}入手(本回合不费气)`);
            } },
            'c33': { id: 'c33', name: '流星落月', cost: 1, type: '仄', typeClass: 'type-ze', desc: `${K.GF}，对敌人的溢出${K.SH}连锁溅射到随机其他敌人`, rarity: 'mid', cardType: '功卡', effect: () => { State.combat.liuXingLuoYue = true; Game.showToast('流星落月：劲力可泄向邻敌'); } },
            'c34': { id: 'c34', name: '坚壁清野', cost: 2, type: '仄', typeClass: 'type-ze', desc: `${K.GF}，本场战斗每回合可跨回合至多保留15点${K.CSHOU}(不足15则按实际持守保留)`, rarity: 'mid', cardType: '功卡', effect: () => { State.combat.player.jianBiQingYe = true; Game.showToast('坚壁清野：阵脚稳住'); } },
            ...
            'c36': { id: 'c36', name: '封刀挂剑', cost: 1, type: '仄', typeClass: 'type-ze', desc: `${K.GF}，你每失去力时，${K.FX}抽取2张卡牌`, rarity: 'mid', cardType: '功卡', effect: () => { State.combat.player.fengDao = true; Game.showToast('封刀挂剑：刀入鞘而意未绝'); } },
            'c37': { id: 'c37', name: '七步成诗', cost: 0, type: '平', typeClass: 'type-ping', toExhaust: true, desc: `打出后进入${K.CS}，本场战斗必将获得随机1份诗句残篇`, rarity: 'mid', cardType: '功卡', effect: () => {
                ...
                if (!avail.length) { Game.showToast('七步成诗：人间残句已尽，无可再得'); return; }
                State.combat.qibuPoetryId = avail[rand(0, avail.length - 1)];
                Game.showToast('七步成诗：战后于犒赏中领取残句');
            } },
            ...
            'c39': { id: 'c39', name: '拔山扛鼎', cost: 1, type: '仄', typeClass: 'type-ze', desc: `${K.GF}，本场战斗你获得4点力`, rarity: 'mid', cardType: '功卡', effect: () => { State.combat.player.combatStr += 4; Game.updateUI(); Game.showToast('拔山扛鼎：本场武力 +4'); } },
            ...
            'c44': { id: 'c44', name: '唇枪舌剑', cost: 3, type: '仄', typeClass: 'type-ze', atkBase: -5, desc: `${K.GF}，你每打出一张仄牌，就对随机一名敌人造成{V_ATK}点${K.SH}(本场仄牌数见状态栏)`, rarity: 'high', cardType: '功卡', effect: () => { State.combat.player.chunQiang = true; Game.showToast('唇枪舌剑：口舌亦可伤人'); } },
            'c45': { id: 'c45', name: '固若金汤', cost: 3, type: '平', typeClass: 'type-ping', defBase: -4, desc: `${K.GF}，你每打出一张平牌，获得{V_DEF}点${K.CSHOU}(本场平牌数见状态栏)`, rarity: 'high', cardType: '功卡', effect: () => { State.combat.player.guRuo = true; Game.showToast('固若金汤：壁立难撼'); } },
            'c46': { id: 'c46', name: '案剑瞋目', cost: 0, type: '仄', typeClass: 'type-ze', toExhaust: true, desc: `打出后进入${K.CS}，获得3点气，${K.FX}抽取3张牌，本场战斗 +3 力；历经三轮敌方回合后暴毙`, rarity: 'high', cardType: '功卡', effect: () => {
                ...
                Game.updateUI();
                Game.showToast('案剑瞋目：三轮敌动之后，气竭而亡');
            } },
            'c47': { id: 'c47', name: '刀光剑影', cost: 2, type: '仄', typeClass: 'type-ze', desc: `${K.GF}，名字含「剑」的牌打出时再结算一次效果(不含本功法；每张牌每回合仅追加一次)`, rarity: 'high', cardType: '功卡', effect: () => { State.combat.player.daoGuang = true; Game.showToast('刀光剑影：剑气纵横'); } },
            'c48': { id: 'c48', name: '金蝉脱壳', cost: 3, type: '仄', typeClass: 'type-ze', toExhaust: true, desc: `打出后进入${K.CS}，将你的血量和状态回退至上回合(不影响敌方)`, rarity: 'high', cardType: '功卡', effect: () => {
                ...
                if (!snap) { Game.showToast('金蝉脱壳：无可倒退的一步'); return; }
                ...
                Game.updateUI();
                Combat.renderHand();
                Game.showToast('金蝉脱壳：身形退回上一回合之初');
            } },
