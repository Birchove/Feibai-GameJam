const CardDB = {
            ...
            'c8': { id: 'c8', name: '水调歌头', cost: 2, type: '平', typeClass: 'type-ping', atkBase: 0, desc: `造成{V_ATK}点${K.SH}2次，抽取(1+轻功)张卡牌`, rarity: 'mid', cardType: '词牌', effect: () => { Combat.dealDmg(0); Game.deferForRun(() => Combat.dealDmg(0), 200, { requireCombat: true }); Combat.draw(1 + State.agi); } },
            ...
            'c15': { id: 'c15', name: '撩剑', cost: 1, type: '仄', typeClass: 'type-ze', atkBase: 4, isFixed: true, desc: `${K.FX}造成{V_ATK}点${K.SH}3次`, isAttack: true, rarity: 'low', cardType: '武卡', effect: () => { Combat.dealDmg(4, true); Game.deferForRun(() => Combat.dealDmg(4, true), 150, { requireCombat: true }); Game.deferForRun(() => Combat.dealDmg(4, true), 300, { requireCombat: true }); } },
            ...
            'c21': { id: 'c21', name: '双斩', cost: 1, type: '仄', typeClass: 'type-ze', atkBase: -3, desc: `造成{V_ATK}点${K.SH}两次`, isAttack: true, rarity: 'low', cardType: '武卡', effect: () => { Combat.dealDmg(-3); Game.deferForRun(() => Combat.dealDmg(-3), 200, { requireCombat: true }); } },
            ...
            'c28': { id: 'c28', name: '白虹贯日', cost: 1, type: '平', typeClass: 'type-ping', toExhaust: true, atkBase: 5, desc: `打出后进入${K.CS}，造成{V_ATK}点${K.SH}，对血量最高敌人造成双倍${K.SH}`, isAttack: true, rarity: 'mid', cardType: '武卡', effect: () => {
                ...
                if (liv0.length === 1) {
                    ...
                    Game.deferForRun(() => Combat.dealDmg(5, false, idx), 120, { requireCombat: true });
                    return;
                }
                Combat.dealDmgAll(5);
            ...
            'c42': { id: 'c42', name: '城焚烬余', cost: 2, type: '平', typeClass: 'type-ping', toExhaust: true, atkBase: -3, desc: `打出后进入${K.CS}，手牌全部放入${K.CS}，对所有敌人造成{V_ATK}点${K.SH}X次(X为${K.CS}牌数)`, isAttack: true, rarity: 'high', cardType: '武卡', effect: () => {
                ...
                for (let i = 0; i < X; i++) {
                    Game.deferForRun(() => Combat.dealDmgAll(-3), i * 200, { requireCombat: true });
                }
            } },
